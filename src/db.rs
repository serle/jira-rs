...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState, Error> {
        ...
        let res: Result<DBState, serde_json::Error> = serde_json::from_str(&json_data);
        res.map_err(|e| Error::msg(e.to_string()))
    }

    fn write_db(&self, db_state: &DBState) -> Result<(), Error> {
        let json_data  = serde_json::to_string_pretty(db_state)?;
        let res = fs::write(self.file_path.clone(), json_data);
        res.map_err(|e| Error::msg(e.to_string()))
    }
}
