...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let new_item_id = db.last_item_id + 1;

        db.last_item_id = new_item_id;
        db.epics.insert(new_item_id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let new_item_id = db.last_item_id + 1;
        db.last_item_id = new_item_id;
        let epic = db.epics.get_mut(&epic_id).ok_or(anyhow!("Epic {} not found", epic_id))?;        
        epic.stories.push(new_item_id);
        db.stories.insert(new_item_id, story);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut db = self.read_db()?;
        let epic = db.epics.get_mut(&epic_id).ok_or(anyhow!("Epic {} not found", epic_id))?;
        // remove any stories in the epic
        ...
        // return turn the last_item_id
        if db.last_item_id == epic_id {
            db.last_item_id = epic_id - 1;
        }        
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        let mut db = self.read_db()?;
        let epic = db.epics.get_mut(&epic_id).ok_or(anyhow!("Epic {} not found", epic_id))?;        
        // remove the story from the given epic
        epic.stories.retain(|v| v != &story_id);
        
        // remove the stroy from the database
        db.stories.remove(&story_id).ok_or(anyhow!("Story {} not found", epic_id))?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut db = self.read_db()?;
        let story = db.stories.get_mut(&story_id).ok_or(anyhow!("Story {} not found", story_id))?;        
